	...
	<tbody>
		<c:forEach items="${pagehelper.list}" var="brand">
			<tr>
				...
				<td>
					<c:if test="${categoryBrand.cbrand_status==1}">
						有效
					</c:if>
					<c:if test="${categoryBrand.cbrand_status==2}">
						无效
					</c:if>
					<c:if test="${categoryBrand.cbrand_status!=1 && categoryBrand.cbrand_status!=2}">
						未知
					</c:if>
				</td>
				<td>
					${categoryBrand.sort_num}
				</td>
				<td class="center ">
