 ...
 * 查看详细信息
 */
function selectDetails(roleId){
	var url = "front/role/selectUpdateById";
	...
	$.post(url, params, function(res){
		...
		if(res!=null){
			...
			if(resp.result_code=="success"){
				...
				$("#role-des").val(role.roleDes);//角色描述
				
				$("#edit-tab").text("编辑角色");//修改选项卡标题为编辑角色
...
function updateTabTitle(){
	$("#edit-tab").text("新增角色");//修改选项卡标题为新增角色
}
...
function saveFun(){
	...
	if(roleId==null || roleId==""){
		url = "front/role/insert";
	}else{
		url = "front/role/updateById";
	}
...
function reloadInfoFun(){
	//操作成功后重新加载
	var href = "front/role/selectItem?pagehelperFun=clickPageBtnRequestFun";
...
function clickPageBtnRequestFun(params){
	var action = "front/role/selectItem";
