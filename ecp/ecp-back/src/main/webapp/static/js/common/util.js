(function(window) {
	...
	util.loading = function() {
		...
		if(modalobj.length == 0) {
			...
			html = 
				...
				'		<img style="width:100px; height:100px; margin-top:200px;" src="./static/images/loading.gif" title="正在努力加载...">'+
				'	</div>'+
				'</div>';
			modalobj.html(html);
		}
	...
	util.dialog = function(title, content, footer, options) {
		...
		if(modalobj.length == 0) {
			$(document.body).append('<div id="' + options.containerName + '" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static"></div>');
...
//				p = '<p><a href="' + redirect + '" target="main" data-dismiss="modal" aria-hidden="true">如果你的浏览器在 <span id="timeout"></span> 秒后没有自动跳转，请点击此链接</a></p>';
				p = '<p><a href="javascript:void(0);" onclick="util.redirect('+redirect+');" target="main" data-dismiss="modal" aria-hidden="true">如果你的浏览器在 <span id="timeout"></span> 秒后没有自动跳转，请点击此链接</a></p>';
			}
		}
		var content = 
			'			<i class="pull-left fa fa-4x fa-'+icons[type]+'"></i>'+
...
//						window.location.href = redirect;
	...
	util.redirect=function(redirect){
		$('#content_div_id').load(redirect);
	};
